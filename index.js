...
async function run() {
  try {
    const usersCollection = client.db('RentHomeDb').collection('users')
    const roomsCollection = client.db('RentHomeDb').collection('rooms')
    const bookingsCollection = client.db('RentHomeDb').collection('bookings')
...
app.get('/', (req, res) => {
  res.send('Hello from RentHome Server..')
})

app.listen(port, () => {
  console.log(`RentHome is running on port ${port}`)
})
